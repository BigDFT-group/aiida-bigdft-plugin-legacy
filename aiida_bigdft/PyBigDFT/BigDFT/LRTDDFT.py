...
def _collection_indexes_iocc(iocc,nvirt,spin=None):
    """
    For each iocc and a selected spin provide the list of couples that are concerned up to nvirt
...
class TransitionMatrix(np.matrix):
    """
    Matrix of Transition Quantities, might be either :class:`CouplingMatrix`  or :class:`TransitionMultipoles`
    ...
    Args:
        ...
        log (Logfile): Instance of the logfile from which the coupling matrix calculation is performed.
            Automatically retrieves the ``norb_occ`` and `norb_virt`` parameters. When ``log`` parameter is present the parameter ``matrix`` is ignored.
...
class CouplingMatrix(TransitionMatrix):
    ...
    def diagonalize(self):
        ...
        Returns:
            (np.matrix, np.matrix):
            tuple of the Eigenvvalues and Eigenvectors of the coupling matrix, as returned by :meth:`numpy.linalg.eigh`.
...
class TransitionMultipoles(TransitionMatrix):
    ...
    def get_transitions(self):
        ...
        Returns:
            numpy.array: Transition quantities multiplied by the square root of the unperturbed transition energy
        """
        newdipole=[]
        for line in self:
...
class Excitations():
    ...
    def __init__(self,cm,tm):
        ...
        self.transitions=tm.get_transitions() #: array: transition quantities coming from the multipoles

        scpr=np.array(np.dot(self.eigenvectors,self.transitions))

        #: array: oscillator strenghts components of the transitions defined as the square of  $\int w_a(\mathbf r) r_i $
        self.oscillator_strenghts= np.array([t**2 for t in scpr[:,0:3]])

        self.avg_os=np.average(self.oscillator_strenghts,axis=1) #: array: average of all the components of the OS

        shp=self.eigenvalues.shape[0]
        self.alpha_prime = 2.0*self.oscillator_strenghts/self.eigenvalues[:,np.newaxis]
        """ array: elements of the integrand of the statical polarizability in the space of the excitations """

        self._indices_for_spin_comparison=self._get_indices_for_spin_comparison()
    ...
    def _get_indices_for_spin_comparison(self):
        ...
        transA=transition_indexes(self.cm.norb_occ,self.cm.norb_virt,inds[0])
        transB=transition_indexes(self.cm.norb_occ,self.cm.norb_virt,inds[1])
    ...
    def identify_singlet_and_triplets(self,tol=1.e-5):
        """
        Find the lookup tables that select the singlets and the triplets among the excitations
        ...
        for exc in range(len(self.eigenvalues)):
            ...
            if sing: sings.append(exc)
            if trip: trips.append(exc)
    ...
    def project_on_spin(self,exc,tol=1.e-8):
        ...
        Returns:
            tuple (bool,bool): couple of booleans indicating if the excitation is a singlet or a triplet respectively
        """
        A,B,zero=[np.ravel(self.eigenvectors[exc,ind]) for ind in self._indices_for_spin_comparison]
        issinglet=np.linalg.norm(A-B) < tol
        istriplet=np.linalg.norm(A+B) < tol
        return issinglet,istriplet
        #print (self.eigenvalues[exc],np.linalg.norm(A),np.linalg.norm(B),A-B,A+B,np.linalg.norm(zero))
    ...
    def determine_occ_energies(self,evals):
        ...
        Args:
            evals (BandArray): the eigenvalues as they are provided (for instance) from a `Logfile` class instance.
        """
        norb_tot=evals[0].info
        norb_occ=self.cm.norb_occ
        occ_energies=[]
        #istart=0
        for ispin,norb in enumerate(norb_occ): #range(len(norb_occ)):
            occ_energies.append(np.array(evals[0][ispin][0:norb])) #istart:istart+norb_occ[ispin]]))
            #istart+=norb_tot[ispin]
        self.occ_energies=occ_energies #: array: energies of the occupied states out of the logfile
    ...
    def plot_alpha(self,**kwargs):
        ...
        Keyword Arguments:
           ...
           **kwargs:
               other arguments that might be passed to the :meth:`plot` method of the :mod:`matplotlib.pyplot` module.

        Returns:
             :mod:`matplotlib.pyplot`: the reference to :mod:`matplotlib.pyplot` module.
        """
    ...
    def lookup(self,group):
        ...
        Args:
            group (str):
               ...
               * ``"all"`` : provides the entire set of excitations (:py:class:`None` instead of the lookup array)
        ...
        if group == 'singlets': slice=self.singlets
        if group == 'triplets': slice=self.triplets
        return slice
    ...
    def dos_dict(self,group='all'):
        ...
        Returns:
            :py:class:`dict`: kewyord arguments that can be passed to the `meth:BigDFT.DoS.append` method of the :class:`DoS.DoS` class
    ...
    def dos(self,group='all',**kwargs):
        ...
        Args:
           ...
           **kwargs: other arguments that might be passed to the :class:`DoS.DoS` instantiation
